...
import jsPDF from "jspdf"; // ✅ Pour le PDF
...
/** 🔹 Renvoie les 7 jours de la semaine courante (dimanche → samedi) */
...
async function openProjSessionForEmp(
  projId,
  empId,
  empName,
  key = todayKey()
...
async function doDepunchWithProject(emp) {
  ...
  await updateDoc(dayRef(emp.id, key), {
    end: serverTimestamp(),
    updatedAt: serverTimestamp(),
  });
}

/* 🔥 Helper : créer un projet + puncher tout de suite
   + demander à la page liste de projets d'ouvrir le questionnaire de création */
async function createAndPunchNewProject(emp) {
  // Projet minimal, ouvert, créé depuis le punch
  ...
  // On démarre tout de suite le temps pour ce projet + travailleur
  await doPunchWithProject(emp, proj);

  // 👉 on indique à PageListeProjet quel projet ouvrir dans le popup questionnaire
  try {
    window.sessionStorage?.setItem("newProjectFromPunch", ref.id);
  } catch {
    // ignore
  }

  // On envoie l’utilisateur à la page liste projets
...
function MiniConfirm({ open, initialProj, projets, onConfirm, onCancel }) {
  ...
  const handleConfirmChoice = () => {
    const chosen = altNone
      ? null
      : projets.find((p) => p.id === altProjId) || null;
    onConfirm(chosen);
  };
      ...
      >
        ...
        {step === "confirm" ? (
          ...
          >
            ...
            <Button variant="success" onClick={handleConfirmDirect}>
              Oui
            </Button>
            <Button variant="danger" onClick={goChoose}>
              Non
            </Button>
          </div>
        ) : (
          <div>
            ...
            >
              ...
              <Button
                ...
                style={{
                  opacity: !altNone && !altProjId ? 0.6 : 1,
                }}
...
function LigneEmploye({ emp, onOpenHistory, setError, projets }) {
  const { sessions, totalMs, hasOpen } = usePresenceToday(
    emp.id,
    setError
  );
  ...
  // 🎨 Jaune permanent si punch SANS projet (segment ouvert avec jobId nul)
  const hasOpenNoProject = useMemo(
    () => sessions.some((s) => !s.end && !s.jobId),
    [sessions]
  );

  const [rowBg, setRowBg] = useState(
    () => styles.row?.background || "white"
  );
  ...
  const handlePunchClick = async (e) => {
    ...
    if (newProjRequested) {
      const ok = window.confirm(
        `Créer un nouveau projet et commencer le temps tout de suite pour ${
          emp.nom || "cet employé"
        } ?`
      );
      if (!ok) return;
    ...
    // Cas normal: projet sélectionné ou aucun projet
  ...
  const togglePunch = async () => {
    try {
      ...
      if (present) {
        ...
        const chosenProj = projSel
          ? projets.find((x) => x.id === projSel)
          : null;
  ...
  const handleMouseEnter = () => {
    if (hasOpenNoProject) return; // reste jaune, pas de survol
    setRowBg(styles.rowHover?.background || "#f9fafb");
  };

  const handleMouseLeave = () => {
    if (hasOpenNoProject) return; // reste jaune
    setRowBg(styles.row?.background || "white");
  };
  ...
  return (
    <>
      <tr
        ...
        style={{
          ...styles.row,
          background: hasOpenNoProject ? "#fef08a" : rowBg,
      ...
      {/* Popup confirmation pour le cas normal */}
      <MiniConfirm
        open={confirmOpen}
...
function ClockBadge({ now }) {
  ...
  return (
    ...
    >
      ...
      <div
        style={{ fontSize: 22, fontWeight: 800, letterSpacing: 1 }}
      >
        {heure}
      </div>
    </div>
...
export default function PageAccueil() {
  ...
  // 🔹 Génération du PDF "Horaire" (semaine courante dimanche→samedi, avec total)
  const handleExportHoraire = async () => {
    try {
      ...
      const semaineTitre = `Semaine du ${weekDays[0].date.toLocaleDateString(
        "fr-CA",
        ...
        <Card
          ...
          right={
            ...
            >
              ...
              <Button
                variant="neutral"
                onClick={handleExportHoraire}
                aria-label="Voir l’horaire de la semaine (PDF)"
              >
                Horaire (PDF)
              </Button>
        ...
        >
          <div style={styles.tableWrap}>
            <table style={styles.table}>
              <thead>
                <tr>
                  {["Nom", "Statut", "Total (jour)", "Projet"].map(
                    (h, i) => (
